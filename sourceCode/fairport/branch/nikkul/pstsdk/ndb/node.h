...
public:
    ...
    subnode_nonleaf_block(const shared_db_ptr& db, std::vector<std::pair<node_id, block_id> > subblocks, size_t max_entries)
        : subnode_block(db, block_info(), 1, max_entries), m_subnode_info(std::move(subblocks)), m_child_blocks(m_subnode_info.size())
    { touch(); }
#else
    ...
    subnode_nonleaf_block(const shared_db_ptr& db, const std::vector<std::pair<node_id, block_id> > subblocks, size_t max_entries)
        : subnode_block(db, block_info(), 1, max_entries), m_subnode_info(subblocks), m_child_blocks(m_subnode_info.size())
    { touch(); }
#endif

    
    subnode_nonleaf_block(const subnode_nonleaf_block& other)
        : subnode_block(other.get_db_ptr(), block_info(),1, other.get_max_entries()), m_subnode_info(other.m_subnode_info), m_child_blocks(other.m_child_blocks)
    { touch(); }
...
public:
    ...
    subnode_leaf_block(const shared_db_ptr& db, std::vector<std::pair<node_id, subnode_info >> subnodes, size_t max_entries)
        : subnode_block(db, block_info(), 0, max_entries), m_subnodes(std::move(subnodes))
    { touch(); }
#else
    ...
    subnode_leaf_block(const shared_db_ptr& db, const std::vector<std::pair<node_id, subnode_info >> subnodes, size_t max_entries)
        : subnode_block(db, block_info(), 0, max_entries), m_subnodes(subnodes)
    { touch(); }
#endif

    
    subnode_leaf_block(const subnode_leaf_block& other)
        : subnode_block(other.get_db_ptr(), block_info(), 0, other.get_max_entries()), m_subnodes(other.m_subnodes)
    { touch(); }
...
inline std::tr1::shared_ptr<subnode_block> subnode_nonleaf_block::remove(node_id nID)
{
...
inline std::pair<std::tr1::shared_ptr<subnode_block>, std::tr1::shared_ptr<subnode_block> > subnode_nonleaf_block::insert(node_id nID, const subnode_info& val)
{
...
inline std::tr1::shared_ptr<subnode_block> subnode_nonleaf_block::modify(node_id nID, const subnode_info& val)
{
    std::tr1::shared_ptr<subnode_nonleaf_block> copiedBlock = shared_from_this();
    if(copiedBlock.use_count() > 2)
    {
...
inline std::tr1::shared_ptr<subnode_block> subnode_leaf_block::remove(node_id nID)
{
...
inline std::pair<std::shared_ptr<subnode_block >, std::tr1::shared_ptr<subnode_block > > subnode_leaf_block::insert(node_id nID, const subnode_info& val)
{
...
inline std::tr1::shared_ptr<subnode_block> subnode_leaf_block::modify(node_id nID, const subnode_info& val)
{
    std::tr1::shared_ptr<subnode_leaf_block> copiedBlock = shared_from_this();
    if(copiedBlock.use_count() > 2)
    {
